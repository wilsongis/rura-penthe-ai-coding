---
description: (Legacy Alias) Execute the next pending XML wave from tasks.md using a strict state machine.
scripts:
...
**`/warden.implement`** is a legacy alias for **`/warden.execute`**. It maintains the same strict state machine execution protocol to ensure codebase integrity.
...
- Stop and instruct the user that the wave is securely committed, and they may run `/warden.execute` (or this alias `/warden.implement`) again to process the next wave.
